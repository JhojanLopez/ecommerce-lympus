...
var app = {
    table: null,
    init: function () {
        app.initDatatable('#listaUsuarios'); /*INICIALIZA LA DATATABLE*/
    },
    initDatatable: function (id) {/*obtenemos el id de la tabla y la inicializamos*/
        app.table = $(id).DataTable({
            ajax: {
                url: 'php/usuario/mostrar_usuarios.php',
    ...
    setDataModalEditar(data) {
        ...
        $('#rol').val(data.FK_CodigoRol);
        
        if(data.activo==0){
            $('#desactivar').removeClass('d-none');
           $('#activar').addClass('d-none');
           
           $('#codigoDesactivar').val(data.codigo);
           
        }else{
            $('#activar').removeClass('d-none');
            $('#desactivar').addClass('d-none');
            $('#codigoActivar').val(data.codigo);
